...
const EditPolicy: React.FC<{ id: any }> = ({ id }) => {
  const router = useRouter();
  const [singlePolicy, setSinglePolicy] = useState<FormValues | null>(null);

  useEffect(() => {
    const fetchSinglePolicyData = async () => {
      try {
        const token = localStorage.getItem('authToken');
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/policy/${id}`,
          {
            headers: {
              Authorization: `${token}`,
            },
        ...
        const policy = response.data.result;
        setSinglePolicy({
    ...
    fetchSinglePolicyData();
  }, [id]);
    // Provide default initial values to prevent hook error
    const initialValues: FormValues = singlePolicy || {
      identifier: '',
      title: '',
      content: '',
      status: '',
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        const response = await axios.put(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/policy`,
          {
            id: id,
        ...
        if (response.status === 200) {
          ...
          toast.error('Edit policy failed! Please try again.');
        }
      } catch (error: any) {
        toast.error('Edit policy failed. Please check your credentials.');
      } finally {
        setSubmitting(false);
      }
    },
