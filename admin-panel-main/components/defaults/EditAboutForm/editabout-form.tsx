...
const EditAbout: React.FC<{ id: any }> = ({ id }) => {
  const router = useRouter();
  const [singleCms, setSingleCms] = useState<FormValues | null>(null);
  ...
  useEffect(() => {
    const fetchSingleCmsData = async () => {
      try {
        ...
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/getcms/${id}`,
          {
            headers: {
              Authorization: `${token}`,
            },
        ...
        const cms = response.data.result;
        setSingleCms({
          identifier: cms.identifier || '',
          title: cms.title || '',
          content: cms.content || '',
          status: cms.status || ''
        });
    ...
    const initialValues: FormValues = singleCms || {
      identifier: '',
      title: '',
      content: '',
      status: '',
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        if (response.status === 200) {
          ...
          toast.error('Edit about failed! Please try again.');
        }
      } catch (error: any) {
        toast.error('Edit about failed. Please check your credentials.');
      } finally {
        setSubmitting(false);
      }
    },
