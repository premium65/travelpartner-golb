  ...
  const fetchUserData = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/user?page=${page}&limit=${PAGE_SIZE}&fillter=%7B%7D&sortObj=%7B%7D`,
        {
          headers: {
            Authorization: `${token}`,
          },
...
const createdAt = dayjs(user.createdAt).tz('Asia/Kolkata');
    ...
    (filters.totalBalance === '' || (typeof user.totalBalance === 'number' ? user.totalBalance.toString() : '').includes(filters.totalBalance)) &&
    (filters.pendingAmount === '' || (typeof user.pendingAmount === 'number' ? user.pendingAmount.toString() : '').includes(filters.pendingAmount)) &&
    (filters.totalDeposit === '' || (typeof user.totalDeposit === 'number' ? user.totalDeposit.toString() : '').includes(filters.totalDeposit)) &&
    (filters.totalWithdraw === '' || (typeof user.totalWithdraw === 'number' ? user.totalWithdraw.toString() : '').includes(filters.totalWithdraw)) &&
    ...
    (filters.refCount === '' || (typeof user.refCount === 'number' ? user.refCount.toString() : '').includes(filters.refCount)) &&
    (filters.taskCount === '' || (typeof user.taskCount === 'number' ? user.taskCount.toString() : '').includes(filters.taskCount)) &&
  ...
  const columns: DataTableProps<any>['columns'] = [
    ...
    },
    {
      accessor: '_id',
      title: 'User Asset',
      render: ({ _id }) => (<Link href={`/userAsset/${encryptData(_id, secretKey)}`}><Button variant="filled" color="rgba(12, 1, 110, 1)" className='text-xs'>User Asset</Button></Link>),
    },
    {
      accessor: 'actions',
      title: (
