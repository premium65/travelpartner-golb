...
const AboutListDatatable = () => {
    ...
    const fetchCmsData = async () => {
      try {
        ...
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/cms?page=${page}&limit=10000&fillter=%7B%7D&sortObj=%7B%7D`,
          {
            headers: {
              Authorization: `${token}`,
            },
        ...
        if (response.status === 200 && response.data.success) {
          setAboutData(response.data.result.data);
        ...
        toast.error('An error occurred while fetching cms data.');
      }
    };
    ...
    const filteredData = aboutData.filter((about) => {
      return (
        (filters.createdAt === '' || dayjs(about.createdAt).format('YYYY-MM-DD').includes(filters.createdAt)) &&
        (filters.identifier === '' || about.identifier.toLowerCase().includes(filters.identifier.toLowerCase())) &&
        (filters.title === '' || about.title.toLowerCase().includes(filters.title.toLowerCase())) &&
        (filters.content === '' || about.content.toLowerCase().includes(filters.content.toLowerCase())) &&
        (filters.status === '' || about.status === filters.status) &&
        (filters._id === '' || about._id.toLowerCase().includes(filters._id.toLowerCase()))
      );
    });
    ...
    const paginatedData = () => {
      ...
      const paginationData = filteredData ? filteredData.slice(start, end) : aboutData.slice(start, end)
      return paginationData
    };
    const renderActions: DataTableColumn<any>['render'] = (record) => (
      <>
      {adminProfileData?.role === "superadmin" &&
      <Group gap={4} justify="center" wrap="nowrap">
      <Link href={`/updatecms/${record._id}`}>
       <ActionIcon
          size="xs"
          title='Edit'
          variant="filled"
    ...
    return (
      ...
      <Link href={'/addcms'}>
        <Button
        variant="filled"
        color="rgba(12, 1, 110, 1)"><IconPlus color='white' size={16} className='mr-2'/> Add About</Button></Link>
    </div>}
      <div className='pt-5'>  {aboutData.length > 0 && 
...
export default AboutListDatatable;
